/**
 * @file main.cu
 * @brief Entry point for CUDA launch benchmark: host-sequenced vs dynamic
 * parallelism.
 *
...
void launch_host_sequence_auto(float *buf0, float *buf1, int n, int iters,
                               int blockSize = 256, cudaStream_t stream = 0);

void launch_dp_sequence_auto(float *buf0, float *buf1, int n, int iters,
                             int blockSize = 256, cudaStream_t stream = 0);
} // namespace bench
...
struct Args {
  ...
  int iters = 128; // arithmetic work per thread
  int runs = 1000; // measured repetitions per strategy
  int warmup = 10; // warmup runs per strategy
...
int main(int argc, char **argv) {
  ...
  if (args.device < 0 || args.device >= deviceCount) {
    std::cerr << "Invalid --device " << args.device << " (have " << deviceCount
              << " devices)" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  check_cuda(cudaGetDeviceProperties(&props, args.device),
             "cudaGetDeviceProperties");
  const int sm = props.major * 10 + props.minor;
  const bool dp_supported =
      (props.major > 3) || (props.major == 3 && props.minor >= 5);

  std::cout << "Device: " << props.name << " (SM " << props.major << "."
            << props.minor << ", " << props.multiProcessorCount << " SMs)"
            << std::endl;
  std::cout << "Settings: n=" << args.n << " iters=" << args.iters
  ...
  for (int i = 0; i < args.warmup; ++i) {
    bench::launch_host_sequence_auto(d_buf0, d_buf1, args.n, args.iters,
                                     args.block, stream);
  }
  check_cuda(cudaStreamSynchronize(stream),
             "cudaStreamSynchronize (host warmup)");
  ...
  if (dp_supported) {
    for (int i = 0; i < args.warmup; ++i) {
      bench::launch_dp_sequence_auto(d_buf0, d_buf1, args.n, args.iters,
                                     args.block, stream);
    }
    check_cuda(cudaStreamSynchronize(stream),
               "cudaStreamSynchronize (dp warmup)");
    ...
    std::cout << "Dynamic parallelism not supported on this device (requires "
                 "SM 3.5+)."
              << std::endl;
  }
  ...
  std::cout << "\nHost-sequenced avg/min/max (ms): " << mean(host_ms) << " / "
            << minv(host_ms) << " / " << maxv(host_ms) << std::endl;
  ...
  if (dp_ran) {
    ...
    std::cout << "\nDynamic-parallelism avg/min/max (ms): " << mean(dp_ms)
              << " / " << minv(dp_ms) << " / " << maxv(dp_ms) << std::endl;
